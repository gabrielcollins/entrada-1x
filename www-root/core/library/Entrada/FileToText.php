...
class FileToText
{
	...
	public static function decode($filename = null, $extension = null)
	{
		...
		{
			case 'doc':
				return FileToText::decodeDOC($filename);
			
			case 'docx':
				return FileToText::decodeDOCX($filename);
			
			case 'ppt':
				return FileToText::decodePPT($filename);

			case 'pptx':
				return FileToText::decodePPTX($filename);

			case 'pdf':
				return FileToText::decodePDF($filename);
	...
	{ 
		...
		{ 
			...
			{ 
				...
				$options = FileToText::getObjectOptions($currentObject); 
				...
				$data = FileToText::getDecodedStream($stream, $options);  
				...
				{ 
	                if (preg_match_all("#BT[\n|\r](.*)ET[\n|\r]#ismU", $data, $textContainers)) 
	                {
						...
						FileToText::getDirtyTexts($texts, $textContainers); 
					} 
					else
					{ 
						FileToText::getCharTransformations($transformations, $data);
					} 
				} 
			} 
		} 
		...
		return FileToText::getTextUsingTransformations($texts, $transformations);
	}
		...
		else 
		{
			...
			foreach ($options as $key => $value) 
			{
				if ($key == "ASCIIHexDecode")
					$_stream = FileToText::decodeAsciiHex($_stream);
				if ($key == "ASCII85Decode")
					$_stream = FileToText::decodeAscii85($_stream);
				if ($key == "FlateDecode")
					$_stream = FileToText::decodeFlate($_stream);
				if ($key == "Crypt") {} // TO DO
			}
		...
		for ($i = 0; $i < count($texts); $i++) 
		{
			...
			for ($j = 0; $j < strlen($texts[$i]); $j++) 
			{
				...
				switch($c) 
				{
					...
					case ">":
						$hexs = str_split($hex, FileToText::$multibyte); // 2 or 4 (UTF8 or ISO)
					...
					case "\\":
						...
						elseif ($c2 >= '0' && $c2 <= '9') 
						{
							...
							$j += strlen($oct) - 1;
							$plain .= html_entity_decode("&#".octdec($oct).";", FileToText::$convertquotes);
						}
						$j++;
						break;

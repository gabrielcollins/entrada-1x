...
if(!defined("PARENT_INCLUDED")) {
	...
	define("IN_PAYMENTS",	true);
	$BREADCRUMB[] = array("url" => ENTRADA_URL."/admin/payments", "title" => $MODULES[strtolower($MODULE)]["title"]);
	$BREADCRUMB[] = array("url" => ENTRADA_RELATIVE."/admin/payments", "title" => "Payments");	
	...
	if (($router) && ($router->initRoute())) {
		...
		if ($ENTRADA_ACL->amIAllowed("eventcontent", "update", false)) {
			...
			$sidebar_html .= "	<li><a href=\"".ENTRADA_RELATIVE."/payments".(($PAYMENT_ID) ? "?".replace_query(array("id" => $PAYMENT_ID, "action" => false, "section" => false)) : "")."\"><img src=\"".ENTRADA_RELATIVE."/images/checkbox-off.gif\" alt=\"\" /> <span>Student View</span></a></li>\n";
			if ($admin_wording) {
				$sidebar_html .= "<li><a href=\"".ENTRADA_RELATIVE."/admin/payments".(($PAYMENT_ID) ? "?".replace_query(array("id" => $PAYMENT_ID, "action" => "manage")) : "")."\"><img src=\"".ENTRADA_RELATIVE."/images/checkbox-on.gif\" alt=\"\" /> <span>".html_encode($admin_wording)."</span></a></li>\n";
			}
			$sidebar_html .= "</ul>\n";

...
if ((!defined("PARENT_INCLUDED")) || (!defined("IN_CONFIGURATION"))) {
	...
	$BREADCRUMB[] = array("url" => ENTRADA_URL."/admin/settings/organisations/manage/payments?".replace_query(array("section" => "add"))."&amp;org=".$ORGANISATION_ID, "title" => "Add Payment Method");
	...
	switch ($STEP) {
		case 2 :
			...
			if (!$ERROR) {
				...
				if ($db->AutoExecute("payment_options", $PROCESSED, "INSERT")) {
					if ($POPTION_ID = $db->Insert_Id()) {
						...
						application_log("error", "There was an error inserting an payment_method. Database said: ".$db->ErrorMsg());
					}
				} else {
					$ERROR++;
					$ERRORSTR[] = "There was a problem inserting this payment method into the system. The system administrator was informed of this error; please try again later.";
	...
	switch ($STEP) {
		...
		default:	
			...
			?>
			<form action="<?php echo ENTRADA_URL."/admin/settings/organisations/manage/payments"."?".replace_query(array("action" => "add", "step" => 2))."&org=".$ORGANISATION_ID; ?>" method="post">
			...
			<thead>
				<tr>
					<td colspan="2"><h1>Add Payment Method</h1></td>
				</tr>
			</thead>
			<tfoot>
				<tr>
